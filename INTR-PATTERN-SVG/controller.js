...
export default function controller() {
  ...
  if (gamepad) {
    ...
    if (buttons[3].pressed) {
      document.getElementById('round').value = parseFloat(document.getElementById('round').value) + 0.4;
    }
    if (buttons[0].pressed) {
      ...
      // 只有当当前值大于0.4时，才允许减小
      if (currentValue > 0.4) {
        document.getElementById('round').value = currentValue - 0.4;
      } else {
        document.getElementById('round').value = 0; // 保证不低于0
      }
    }

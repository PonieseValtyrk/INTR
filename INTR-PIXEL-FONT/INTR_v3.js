...
let textColor = 'rgb(0,0,0)', backgroundColor = 'rgb(255,255,255)';
let inputText;
let position;
let boolean;
...
function applyParameters() {
  ...
  boolean = document.getElementById('boolean').value;
    ...
    for (const char of line) {
      ...
      for (let i = 0; i < letter.length; i++) {
        for (let j = 0; j < letter[i].length; j++) {
          ...
          renderer.fill(isActive ? textColor : backColor);
          renderer.rect(
            isActive ? realDotX : realUnusedDotX,
            isActive ? realDotY : realUnusedDotY,
...
function saveFont() {
  ...
  let xSize = xDotSize * 10;
  let ySize = yDotSize * 10;
  let xSize2 = xDotUnusedSize * 10;
  let ySize2 = yDotUnusedSize * 10;
  ...
  for (let char in letters) {
    ...
    // xDist = 25 + 125 * Math.random()
    // yDist = 25 + 125 * Math.random()
    // xSize = 50 + 150 * Math.random()
    // ySize = 50 + 150 * Math.random()
    // xSize2 = 25 + 75 * Math.random()
    // ySize2 = 25 + 75 * Math.random()
    if (char === ' ') {
      const Glyph = new opentype.Glyph({
        name: char,
        unicode: char.charCodeAt(0),
        advanceWidth: xDist * letters[char][0].length + 1 + xSize,
      });
      glyphs.push(Glyph)
    } else {
      var svg = matrixToSvg(letters[char], xDist, yDist, xSize, ySize, xSize2, ySize2, boolean, position)
      var dPath = svgToD(svg, yDist * 8)
  ...
  font.metas.XDSZ = `${xDotSize.toFixed(1)}`
  font.metas.YDSZ = `${yDotSize.toFixed(1)}`
  font.metas.XDUS = `${xDotUnusedSize.toFixed(1)}`
  font.metas.YDUS = `${yDotUnusedSize.toFixed(1)}`
  font.metas.XITV = `${xInterval.toFixed(1)}`
  font.metas.YITV = `${yInterval.toFixed(1)}`
  font.metas.BOOL = `${boolean}`
  font.metas.PSTN = `${position}`
...
window.addEventListener('load', () => {
  ...
  const paramInputs = ['xInterval', 'yInterval', 'xDotSize', 'yDotSize',
    'xDotUnusedSize', 'yDotUnusedSize', 'inputText', 'Spacing', 'boolean'];
